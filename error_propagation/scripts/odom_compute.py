...
def transition_model(x, u):
        ...
        pose.position.x = x.x
        pose.position.y = x.y
        quat = tf.transformations.quaternion_from_euler(0.0, 0.0, x.theta)
        pose.orientation = Quaternion(quat[0], quat[1], quat[2], quat[3])

        g = jacobians.getG(pose, u)
        g_transpose = g.transpose()

        v = jacobians.getV(pose, u)
        v_transpose = v.transpose()

        left_error = .05 * u[0]
        right_error = .05 * u[1]
        m = np.matrix([ [left_error, 0], [0, right_error] ])
...
def build_odom_msg(state, odom):
        ...
        odom.pose.covariance = state.cov
